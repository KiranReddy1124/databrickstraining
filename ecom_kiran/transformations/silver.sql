CREATE OR REFRESH STREAMING TABLE kiran_silver.sales_cleaned
(
  CONSTRAINT valid_order_id EXPECT (order_id IS NOT NULL) ON VIOLATION DROP ROW
)
AS
SELECT DISTINCT * EXCEPT (_rescued_data, ingestion_date)
FROM STREAM(kiran_bronze.sales);

-- Create and populate the target table.
CREATE OR REFRESH STREAMING TABLE kiran_silver.product_scd1;

CREATE FLOW product_flow AS AUTO CDC INTO
  kiran_silver.product_scd1
FROM
  stream(kiran_bronze.products)
KEYS
  (product_id)
